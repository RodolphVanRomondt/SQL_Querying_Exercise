-- Comments in SQL Start with dash-dash --

-- app with an ID of 1880 --
SELECT * FROM analytics WHERE id = 1880;

-- last updated on August 01, 2018 --
SELECT id, app_name FROM analytics WHERE last_updated = '2018-08-01';

--  number of apps in each category --
SELECT category, COUNT(*) FROM analytics GROUP BY category;

-- top 5 most-reviewed apps and the number of reviews --
SELECT app_name, reviews FROM analytics ORDER BY reviews DESC LIMIT 5;

-- most reviews with a rating greater than equal to 4.8 --
SELECT app_name, reviews , rating FROM analytics WHERE rating >= 4.8 ORDER BY reviews DESC LIMIT 1;

-- average rating for each category ordered by the highest rated to lowest rated --
SELECT category, AVG(rating) FROM analytics GROUP BY category ORDER BY AVG(rating) DESC;

-- most expensive app with a rating that’s less than 3 --
SELECT app_name, price, rating FROM analytics WHERE rating < 3 ORDER BY price DESC LIMIT 1;

-- apps with a min install not exceeding 50, that have a rating --
SELECT min_installs, rating FROM analytics WHERE min_installs <= 50 AND rating > 0 ORDER BY rating DESC, min_installs DESC;

-- apps that are rated less than 3 with at least 10000 reviews --
SELECT app_name FROM analytics WHERE rating < 3 AND reviews >= 10000;

-- top 10 most-reviewed apps that cost between 10 cents and a dollar --
SELECT reviews, price FROM analytics WHERE price BETWEEN 0.1 AND 1 ORDER BY reviews DESC LIMIT 10;

-- most out of date app --
SELECT app_name, last_updated FROM analytics ORDER BY last_updated LIMIT 1;

-- most expensive app --
SELECT * FROM analytics WHERE price = (SELECT MAX(price) FROM analytics);

-- all the reviews --
SELECT SUM(reviews) FROM analytics;

-- categories that have more than 300 apps --
SELECT category, COUNT(*) FROM analytics GROUP BY category HAVING COUNT(*) > 300;

-- app that has the highest proportion of min_installs to reviews, among apps that have been installed at least 100,000 times --
SELECT app_name, reviews, min_installs, min_installs/reviews AS proportion FROM analytics WHERE min_installs >= 100000 ORDER BY proportion DESC LIMIT 1;
